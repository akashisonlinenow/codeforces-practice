...
// Code
void ak()
{                              // referred from coding dynamo utube
    ll n, b, one = 0, two = 0; // one->no. of ones, two-> no. of twos
    cin >> n;
    vector<ll> v;
    for (ll i = 0; i < n; i++)
    {
        ...
        {
            two++;
            v.push_back(i + 1); // pushing all index no. of 2 present
        }
    }
    if (two & 1)
    {
    ...
    {
        if (two == 0) // if only 1s are present
        {
            cout << 1;
        }
        else // if no. of 2s are even
        {
            ll si = v.size() / 2;
            si--;
            cout << v[si];
